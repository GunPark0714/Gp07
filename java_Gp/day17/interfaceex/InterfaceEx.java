...
public class InterfaceEx {
	...
	public static void main(String[] args) {
		...
		run.run();	
		run.load();
		//구현클래스로 생성한 Runinterface가 관리
		RunInterface runInterface = new Run();
		runInterface.run();
		//RunInterface에는 load가 추상메서드로 존재하지 않기때문에 에러가 발생
		//runInterface.load(); 
		//아래 코드는 Run클래스의 객체로 클래스변환이 되었기때문에
		//load()메서드를 호출할 수 있음.
	...
	/*인터페이스 매개변수 다형성
	 * -매개변수를 인터페이스의 객체로 선언하면, 호출할때 매개변수로
	 * 구현 클래스들의 객체들이 올 수 있다
