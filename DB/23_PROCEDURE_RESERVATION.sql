...
BEGIN 
    ...
    */
    ...
    SET _RV_NUM = CONCAT(
    -- NOW() : YYYY-MM-DD HH:MM:SS = > YYYYMMDDHHMM reservation
   ...
   OPEN CUR;
    READ_LOOP:LOOP
        ...
        -- 성인을 먼저하고 청소년을 먼저하는 건 중요하지 않음
        -- 성인먼저 좌석순으로 배치를 하고, 이후에 청소년 을 좌석순으로 배치
        -- 성인수가 0이 아니면 주어진 좌석은 성인좌석
		IF _ADULT_COUNT != 0 THEN 
			SET _PR_NUM = (SELECT PR_NUM FROM PRICE WHERE PR_TYPE='성인');
            SET _ADULT_COUNT = _ADULT_COUNT - 1;
		ELSE -- 성인수가 0이면 주어진 좌석은 청소년좌석
	...
	SET 
        ...
        SELECT 
			SC_TOTAL SEAT - SUM(RV_ADULT + RV_TEENAGER)
            FROM
				RESERVATION
