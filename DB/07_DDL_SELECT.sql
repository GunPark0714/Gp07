...
/* 
select 속성명, ..., 속성명n from 테이블명 where 조건식;
...
select * from student where major <> '컴퓨터공학과';
-- 학번이 2023135001인 학생이 듣는 수강정보를 조회하는 쿼리를 작성
...
select * from student where name like '유__';
...
-- 과목명이 글쓰기인 과목의 수강정보를 조회하는 쿼리를
select * from course where subject_code = (select code from subject where title = '글쓰기');

-- 학점이 2학점 또는 3학점이상인 과목의 수강 정보를 조회
select * from course where subject-code = any(select code from subject where point in(2,3)
);
...
select major, count(num) as 학생수 from student group by major;
